...
def prepare(env, target, options, ignifuga_src, python_build):
    if options.bare:
        ...
        ignifuga_module = "\nignifuga %s -I%s -I%s -I%s -I%s -L%s %s -lstdc++\n" % (' '.join(ignifuga_src), target.builds.IGNIFUGA, join(target.builds.IGNIFUGA, 'spine'), join(target.dist, 'include'), join(target.dist, 'include', 'freetype2'), join(target.dist, 'libs'), sdlflags)
