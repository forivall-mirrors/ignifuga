...
def prepare(env, target, options, ignifuga_src, python_build):
    ...
    if options.bare:
        ...
        ignifuga_module = "\n%s %s -I%s -I%s -I%s -lturbojpeg -lSDL2_ttf -lSDL2_image -lSDL2_mixer -lvorbisfile -lvorbis -logg %s %s -lpng12 -ljpeg -lz -lws2_32 -lwsock32 -lstdc++ -lgcc\n" % (options.modulename, ' '.join(ignifuga_src), target.builds.IGNIFUGA, join(target.builds.IGNIFUGA, 'spine'), join(target.builds.PYTHON, 'Include'), sdlflags, freetypeflags)
